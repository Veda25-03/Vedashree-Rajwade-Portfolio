...
const Analytics = () => {
  ...
  const maxDonors = Math.max(...barChartData.map(d => d.donors));
  const minDonors = Math.min(...barChartData.map(d => d.donors));

  const getBarColor = (donors) => {
    if (donors === maxDonors) return '#22c55e'; // Green
    if (donors === minDonors) return '#ef4444'; // Red
  ...
  return (
    <div className="p-6 space-y-8">
      ...
      <div className="bg-white rounded-xl shadow p-5">
        <h2 className="text-lg font-semibold mb-4 text-gray-800">📈 Overview Chart (Line)</h2>
      ...
      {/* Pie Chart Card */}
      <div className="bg-white rounded-xl shadow p-5">
        <h2 className="text-lg font-semibold mb-4 text-gray-800">🩸 Blood Group Chart (Pie)</h2>
        <ResponsiveContainer width="100%" height={300}>
          <PieChart>
            <Pie
      ...
      {/* Bar Chart Card */}
      <div className="bg-white rounded-xl shadow p-5">
        <h2 className="text-lg font-semibold mb-4 text-gray-800">🏥 Donors by Hospital (Bar)</h2>
        <ResponsiveContainer width="100%" height={300}>
